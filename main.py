...
class CustomClassifier(torch.nn.Module):
    def __init__(self, model, input_dim, output_dim, 
        ...
        # self.channel_bn = torch.nn.BatchNorm1d(input_dim,affine=False,)
        # self.layers = torch.nn.Sequential(torch.nn.Linear(input_dim, output_dim))

        # self.classifier = nn.Sequential(
        #     torch.nn.BatchNorm1d(input_dim,affine=False,),
        #     # torch.nn.Linear(input_dim, input_dim*4),
        #     nn.Dropout(0.1), # 加入Dropout
        #     nn.ReLU(),
        #     # torch.nn.AdaptiveAvgPool1d(input_dim),
        ...
        #     nn.Linear(input_dim, 256),   # 较小层宽,根据特征数适当选择
        #     nn.ReLU(),
        ...
        #     nn.BatchNorm1d(64),
        #     nn.Dropout(0.2),
        #     nn.Linear(64, output_dim)       # dim类问题
        # )
        ...
        #     nn.AdaptiveAvgPool2d(1, ),
        #     Flatten(start_dim=1, end_dim=-1),
        #     nn.Linear(in_features=1536, out_features=output_dim, bias=True)
        # )

        # for vit -tiny patch
        self.classifier = nn.Sequential(
            nn.LayerNorm(torch.Size([384]), eps=1e-06, elementwise_affine=True),
            nn.Linear(in_features=384, out_features=768),
            nn.Linear(in_features=768,out_features=output_dim, bias=True)

        )

    def forward(self, img, dataset_id=None):
        # TODO: how to leverage dataset_source in training and infernece stage?
        # TODO: better classifier given qwq/
        pdtype = img.dtype
...
def main(args):
    ...
    print(f"train dataset created for path {args.dataset_list}")
    dataset_train, args.nb_classes = build_dataset(is_train=True, args=args)
    print(f"val dataset created for path {args.dataset_list}")
    ...
    # model = timm.create_model('vit_base_patch16_224', pretrained=True) # .31
    model = timm.create_model('vit_small_r26_s32_224', pretrained=True) # .35
    # model = timm.create_model('vit_small_resnet50d_s16_224', pretrained=True)
    # model = timm.create_model('deit_base_patch16_224', pretrained=True, num_classes=1000)
    ...
    model = CustomClassifier(model, 1536, args.nb_classes, multi_dataset_classes=multi_dataset_classes, known_data_source=args.known_data_source)
